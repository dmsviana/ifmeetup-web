...
const EventDetailsModal = ({ 
  ...
  const {
    ...
    autoLoad: isOpen && !!eventId,
    onStatusChange: (newStatus) => {
      // atualizar estado local do evento se necessário
      if (event && newStatus.participantsCount !== undefined) {
        setEvent(prev => ({
          ...prev,
          currentParticipants: newStatus.participantsCount
        }));
      }
  ...
  // verificar se evento está lotado baseado nos dados atuais
  const isCurrentlyFull = event && participantsCount >= event.maxParticipants;
  
  // determinar se botões devem voltar ao estado normal quando capacidade muda
  const shouldEnableRegistration = event && !isCurrentlyFull && !isEventCompleted && event.status === 'APPROVED';
  ...
  }, [isOpen, eventId, loadEvent, loadUserFeedback]);

  // handler para participação
  const handleParticipationToggle = async () => {
    const result = await toggleRegistration();
    if (result.success) {
      // recarregar dados do evento para atualizar contador
      setTimeout(() => {
  ...
  const handleAttendanceUpdate = () => {
    // recarregar participação para atualizar contadores
    refreshParticipation();
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col h-full max-h-[80vh]">
        {/* Header */}
        <Modal.Header onClose={onClose} className="flex-shrink-0">
          <div className="flex items-center space-x-3">
            {event && (
              <>
              ...
              <div className="flex space-x-3">
                ...
                {event.status === 'APPROVED' && !isEventCompleted && (
                  <button
                    ...
                    disabled={isParticipationLoading || (!canRegister && !isRegistered)}
                    className={`px-4 py-2 rounded-lg font-medium transition-colors focus:ring-2 focus:ring-offset-2 ${
                      isRegistered
                        ? 'bg-red-100 text-red-800 border border-red-200 hover:bg-red-200 focus:ring-red-500'
                        : canRegister
                        ? 'bg-green-600 text-white hover:bg-green-700 focus:ring-green-500'
                        : 'bg-gray-300 text-gray-500 cursor-not-allowed'
                    } ${isParticipationLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
