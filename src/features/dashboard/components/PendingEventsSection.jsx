...
import { LoadingSpinner, ToastContainer } from '../../../shared/components/ui';
...
const PendingEventsSection = ({ onEventProcessed, className = '' }) => {
  ...
  const handleCardReject = useCallback((eventId) => {
    // validate eventId exists and is valid
    if (eventId && pendingEvents.find(e => e.id === eventId)) {
      setSelectedEventId(eventId);
      setShowRejectModal(true);
    } else {
      error('Evento não encontrado ou ID inválido');
    }
  }, [pendingEvents, error]);

  const handleCardViewDetails = useCallback((eventId) => {
    // validate eventId exists and is valid
    if (eventId && pendingEvents.find(e => e.id === eventId)) {
      setSelectedEventId(eventId);
  ...
  const handleRejectModalSubmit = useCallback((rejectionReason) => {
    if (selectedEventId) {
      handleRejectEvent(selectedEventId, rejectionReason);
    }
  }, [selectedEventId, handleRejectEvent]);
  ...
  return (
    ...
    >
      ...
      <RejectEventModal
        ...
        event={selectedEventId ? pendingEvents.find(e => e.id === selectedEventId) : null}
        onClose={handleRejectModalClose}
        onReject={handleRejectModalSubmit}
        isSubmitting={selectedEventId ? processingEvents.has(selectedEventId) : false}
      />
